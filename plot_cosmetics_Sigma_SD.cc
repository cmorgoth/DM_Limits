{
  ...
  mg->GetYaxis()->SetRangeUser(1e-41, 8e-37);
  ...
  TLegend* leg = new TLegend(0.4, 0.75, 0.89, 0.89);//(xmin, ymin, xmax, ymax)
  leg->AddEntry(Ld, "Razor SD u-quark (5 fb^{-1})" ,"l");
  leg->AddEntry(Lu, "Razor SD d-quark (5 fb^{-1})" ,"l");
  leg->AddEntry(Lmono, "Monojer SD combined limit (19.5 fb^{-1})" ,"l");
  leg->SetFillColor(0);
  leg->SetBorderSize(0);
  ...
  pad1->SetLogx();
  pad1->Update();
  C->SaveAs("SD_SigmaLimit.pdf");
  C->SaveAs("SD_SigmaLimit.png");
